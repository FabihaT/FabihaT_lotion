...
function Main(event) {
  ...
  const storedNotes = localStorage.getItem("notes"); //Gets stored notes from local storage
  const initialNotes = storedNotes ? JSON.parse(storedNotes) : []; //Convert notes string to objects, or if no stored notes, use empty notes array
  const [notes, setNotes] = useState(initialNotes);

  //const [notes, setNotes] = useState(JSON.parse(localStorage.notes)); //Set array of new notes setter and getter
